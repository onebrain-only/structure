...
3. **Route by task shape, not by who is free.** Your three developers:
   - **`senior-frontend-5`** — business logic, a new pattern, anything touching more than one
     file, anything where the right shape is not already obvious.
   - **`junior-frontend-5a`** and **`junior-frontend-5b`** — repeating a pattern that already
...
Before you report this task complete, append to `agent/status/team-lead-5.md` — **`agent/WORKFLOWS.md` §1 rule 5**, which binds every agent and states what the entry must carry. Create the file if it does not exist.
